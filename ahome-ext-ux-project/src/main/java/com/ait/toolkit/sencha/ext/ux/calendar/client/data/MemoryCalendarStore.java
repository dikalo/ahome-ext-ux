...
public class MemoryCalendarStore extends Store {

	public MemoryCalendarStore(JavaScriptObject[] storeData) {
		assert storeData.length > 0 : "Store data can not be empty";
		create("", JsoHelper.arrayConvert(storeData));

	}

	public MemoryCalendarStore(List<? extends BaseModel> data) {
		assert !data.isEmpty() : "BaseModel list cannot be empty.";
		// Set<String> fields = new HashSet<String>(data.get(0).getFields());
		init(data);
	}

	public static MemoryCalendarStore from(List<? extends Bean> beans) {
		List<BaseModel> models = new ArrayList<BaseModel>();
		for (Bean bean : beans) {
			models.add(BaseModel.from(bean.getJsObj()));
		}
		return new MemoryCalendarStore(models);
	}
	...
	private native void create(String modelName, JavaScriptObject values)/*-{
		this.@com.ait.toolkit.core.client.JsObject::jsObj = new $wnd.Extensible.calendar.data.MemoryCalendarStore(
				{
					data : values
				});
	}-*/;
